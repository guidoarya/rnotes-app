...
class NoteApp extends React.Component {
  ...
  activeNotes() {
    const dataActiveNote = this.state.notes.filter(
  ...
  archiveNote() {
    const dataArchiveNote = this.state.notes.filter(
      ...
      <>
        ...
        <div className="note-list_container">
          ...
          {this.searchingFilter(
            this.searchingFilter(this.activeNotes())
          ).length === 0 ? (
            <p className="not-found">Your note doesn't exist</p>
          ) : (
            <NoteList
              notes={this.searchingFilter(this.activeNotes())}
              onDelete={this.onDeleteHandler}
              onArchive={this.onArchiveHandler}
              onNoteList={this.onNoteListHandle}
            />
          )}
        </div>
        <div className="note-list_container">
          <h2>Archive List</h2>
          {this.searchingFilter(
            this.searchingFilter(this.archiveNote())
          ).length === 0 ? (
            <p className="not-found">
              Your archived note doesn't exist
            </p>
          ) : (
            <ArchiveList
              notes={this.searchingFilter(this.archiveNote())}
              onDelete={this.onDeleteHandler}
              onNoteList={this.onNoteListHandle}
            />
          )}
