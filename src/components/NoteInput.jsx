...
class NoteInput extends React.Component {
    ...
    onTitleChangeEventHandler(event) {
      ...
      handleWordCount(event){
        ...
        this.setState({[event.target.name]: event.target.value.slice(0, 50)})
      }
        ...
        return(
            <>
                ...
                <form className='note-input' action="/">
                    <input type="text" name="title" value={this.state.title} placeholder='Input title here...' onChange={this.handleWordCount} required/>
                    <p className='max-char'>{this.state.title.length} / {this.state.count}</p>
                    <br />
